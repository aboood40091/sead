...
)
{
    ...
    GX2InitAttribStream(&mAttributes[0], mAttrVertexLocation, 0, 0, GX2_ATTRIB_FORMAT_32_32_32_FLOAT);
    GX2InitAttribStream(&mAttributes[1], mAttrTexCoord0Location, 0, sizeof(f32) * 3, GX2_ATTRIB_FORMAT_32_32_FLOAT);
    GX2InitAttribStream(&mAttributes[2], mAttrColorRateLocation, 0, sizeof(f32) * 5, GX2_ATTRIB_FORMAT_32_32_32_32_FLOAT);
    ...
    {
        // Quad
        mQuadVertexBuf = static_cast<PrimitiveRendererUtil::Vertex*>(heap->alloc(4 * sizeof(PrimitiveRendererUtil::Vertex), GX2_VERTEX_BUFFER_ALIGNMENT));
        ...
        GX2Invalidate(GX2_INVALIDATE_CPU_ATTRIB_BUFFER, mQuadVertexBuf, 4 * sizeof(PrimitiveRendererUtil::Vertex));
    ...
    {
        // Line
        mLineVertexBuf = static_cast<PrimitiveRendererUtil::Vertex*>(heap->alloc(4 * sizeof(PrimitiveRendererUtil::Vertex), GX2_VERTEX_BUFFER_ALIGNMENT));
        ...
        GX2Invalidate(GX2_INVALIDATE_CPU_ATTRIB_BUFFER, mLineVertexBuf, 4 * sizeof(PrimitiveRendererUtil::Vertex));
        GX2Invalidate(GX2_INVALIDATE_CPU_ATTRIB_BUFFER,  mLineIndexBuf, 6 * sizeof(u16));
    }

    {
        // Cube
        mCubeVertexBuf = static_cast<PrimitiveRendererUtil::Vertex*>(heap->alloc(8 * sizeof(PrimitiveRendererUtil::Vertex), GX2_VERTEX_BUFFER_ALIGNMENT));
        mCubeIndexBuf  = static_cast<u16*>(heap->alloc(36 * sizeof(u16), GX2_INDEX_BUFFER_ALIGNMENT));
        PrimitiveRendererUtil::setCubeVertex(mCubeVertexBuf, mCubeIndexBuf);

        GX2Invalidate(GX2_INVALIDATE_CPU_ATTRIB_BUFFER, mCubeVertexBuf,  8 * sizeof(PrimitiveRendererUtil::Vertex));
        GX2Invalidate(GX2_INVALIDATE_CPU_ATTRIB_BUFFER,  mCubeIndexBuf, 36 * sizeof(u16));
    }

    {
        // WireCube
        mWireCubeVertexBuf = static_cast<PrimitiveRendererUtil::Vertex*>(heap->alloc(8 * sizeof(PrimitiveRendererUtil::Vertex), GX2_VERTEX_BUFFER_ALIGNMENT));
        mWireCubeIndexBuf  = static_cast<u16*>(heap->alloc(17 * sizeof(u16), GX2_INDEX_BUFFER_ALIGNMENT));
        PrimitiveRendererUtil::setWireCubeVertex(mWireCubeVertexBuf, mWireCubeIndexBuf);

        GX2Invalidate(GX2_INVALIDATE_CPU_ATTRIB_BUFFER, mWireCubeVertexBuf,  8 * sizeof(PrimitiveRendererUtil::Vertex));
        GX2Invalidate(GX2_INVALIDATE_CPU_ATTRIB_BUFFER,  mWireCubeIndexBuf, 17 * sizeof(u16));
    }

    {
        // SphereS
        mSphereSVertexBuf = static_cast<PrimitiveRendererUtil::Vertex*>(heap->alloc((4*8 + 2) * sizeof(PrimitiveRendererUtil::Vertex), GX2_VERTEX_BUFFER_ALIGNMENT));
        mSphereSIndexBuf  = static_cast<u16*>(heap->alloc((4*8 * 6) * sizeof(u16), GX2_INDEX_BUFFER_ALIGNMENT));
        PrimitiveRendererUtil::setSphereVertex(mSphereSVertexBuf, mSphereSIndexBuf, 8, 4);

        GX2Invalidate(GX2_INVALIDATE_CPU_ATTRIB_BUFFER, mSphereSVertexBuf, (4*8 + 2) * sizeof(PrimitiveRendererUtil::Vertex));
        GX2Invalidate(GX2_INVALIDATE_CPU_ATTRIB_BUFFER,  mSphereSIndexBuf, (4*8 * 6) * sizeof(u16));
    }

    {
        // SphereL
        mSphereLVertexBuf = static_cast<PrimitiveRendererUtil::Vertex*>(heap->alloc((8*16 + 2) * sizeof(PrimitiveRendererUtil::Vertex), GX2_VERTEX_BUFFER_ALIGNMENT));
        mSphereLIndexBuf  = static_cast<u16*>(heap->alloc((8*16 * 6) * sizeof(u16), GX2_INDEX_BUFFER_ALIGNMENT));
        PrimitiveRendererUtil::setSphereVertex(mSphereLVertexBuf, mSphereLIndexBuf, 16, 8);

        GX2Invalidate(GX2_INVALIDATE_CPU_ATTRIB_BUFFER, mSphereLVertexBuf, (8*16 + 2) * sizeof(PrimitiveRendererUtil::Vertex));
        GX2Invalidate(GX2_INVALIDATE_CPU_ATTRIB_BUFFER,  mSphereLIndexBuf, (8*16 * 6) * sizeof(u16));
    }

    {
        // DiskS
        mDiskSVertexBuf = static_cast<PrimitiveRendererUtil::Vertex*>(heap->alloc((16 + 1) * sizeof(PrimitiveRendererUtil::Vertex), GX2_VERTEX_BUFFER_ALIGNMENT));
        mDiskSIndexBuf  = static_cast<u16*>(heap->alloc((8 * 6) * sizeof(u16), GX2_INDEX_BUFFER_ALIGNMENT));
        PrimitiveRendererUtil::setDiskVertex(mDiskSVertexBuf, mDiskSIndexBuf, 16);

        GX2Invalidate(GX2_INVALIDATE_CPU_ATTRIB_BUFFER, mDiskSVertexBuf, (16 + 1) * sizeof(PrimitiveRendererUtil::Vertex));
        GX2Invalidate(GX2_INVALIDATE_CPU_ATTRIB_BUFFER,  mDiskSIndexBuf,  (8 * 6) * sizeof(u16));
    }

    {
        // DiskL
        mDiskLVertexBuf = static_cast<PrimitiveRendererUtil::Vertex*>(heap->alloc((32 + 1) * sizeof(PrimitiveRendererUtil::Vertex), GX2_VERTEX_BUFFER_ALIGNMENT));
        mDiskLIndexBuf  = static_cast<u16*>(heap->alloc((16 * 6) * sizeof(u16), GX2_INDEX_BUFFER_ALIGNMENT));
        PrimitiveRendererUtil::setDiskVertex(mDiskLVertexBuf, mDiskLIndexBuf, 32);

        GX2Invalidate(GX2_INVALIDATE_CPU_ATTRIB_BUFFER, mDiskLVertexBuf, (32 + 1) * sizeof(PrimitiveRendererUtil::Vertex));
        GX2Invalidate(GX2_INVALIDATE_CPU_ATTRIB_BUFFER,  mDiskLIndexBuf, (16 * 6) * sizeof(u16));
    }

    {
        // CircleS
        mCircleSIndexBuf = static_cast<u16*>(heap->alloc(16 * sizeof(u16), GX2_INDEX_BUFFER_ALIGNMENT));
        for (s32 i = 0; i < 16; i++)
    ...
    {
        // CircleL
        mCircleLIndexBuf = static_cast<u16*>(heap->alloc(32 * sizeof(u16), GX2_INDEX_BUFFER_ALIGNMENT));
        for (s32 i = 0; i < 32; i++)
    ...
    {
        // CylinderS
        mCylinderSVertexBuf = static_cast<PrimitiveRendererUtil::Vertex*>(heap->alloc((16*2 + 2) * sizeof(PrimitiveRendererUtil::Vertex), GX2_VERTEX_BUFFER_ALIGNMENT));
        mCylinderSIndexBuf  = static_cast<u16*>(heap->alloc((16 * 12) * sizeof(u16), GX2_INDEX_BUFFER_ALIGNMENT));
        PrimitiveRendererUtil::setCylinderVertex(mCylinderSVertexBuf, mCylinderSIndexBuf, 16);

        GX2Invalidate(GX2_INVALIDATE_CPU_ATTRIB_BUFFER, mCylinderSVertexBuf, (16*2 + 2) * sizeof(PrimitiveRendererUtil::Vertex));
        GX2Invalidate(GX2_INVALIDATE_CPU_ATTRIB_BUFFER,  mCylinderSIndexBuf,  (16 * 12) * sizeof(u16));
    }

    {
        // CylinderL
        mCylinderLVertexBuf = static_cast<PrimitiveRendererUtil::Vertex*>(heap->alloc((32*2 + 2) * sizeof(PrimitiveRendererUtil::Vertex), GX2_VERTEX_BUFFER_ALIGNMENT));
        mCylinderLIndexBuf  = static_cast<u16*>(heap->alloc((32 * 12) * sizeof(u16), GX2_INDEX_BUFFER_ALIGNMENT));
        PrimitiveRendererUtil::setCylinderVertex(mCylinderLVertexBuf, mCylinderLIndexBuf, 32);

        GX2Invalidate(GX2_INVALIDATE_CPU_ATTRIB_BUFFER, mCylinderLVertexBuf, (32*2 + 2) * sizeof(PrimitiveRendererUtil::Vertex));
        GX2Invalidate(GX2_INVALIDATE_CPU_ATTRIB_BUFFER,  mCylinderLIndexBuf,  (32 * 12) * sizeof(u16));
    }
