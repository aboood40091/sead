    ...
    if (k_pad_info.last_read_length > 0 || k_pad_info.last_read_error == KPAD_READ_ERR_NO_DATA)
    {
        ...
        if (status.dev_type != WPAD_DEV_NOT_FOUND)
        {
            ...
            if (status.dev_type == WPAD_DEV_URCC)
            {
                ...
                mPadHold.change(cPadFlag_A,      hold & KPAD_UC_BUTTON_A);
                mPadHold.change(cPadFlag_B,      hold & KPAD_UC_BUTTON_B);
                mPadHold.change(cPadFlag_X,      hold & KPAD_UC_BUTTON_X);
                mPadHold.change(cPadFlag_Y,      hold & KPAD_UC_BUTTON_Y);
                mPadHold.change(cPadFlag_L,      hold & KPAD_UC_TRIGGER_L);
                mPadHold.change(cPadFlag_R,      hold & KPAD_UC_TRIGGER_R);
                mPadHold.change(cPadFlag_C,      hold & KPAD_UC_TRIGGER_ZL);
                mPadHold.change(cPadFlag_Z,      hold & KPAD_UC_TRIGGER_ZR);
                mPadHold.change(cPadFlag_Plus |
                                cPadFlag_Start,  hold & KPAD_UC_BUTTON_PLUS);
                mPadHold.change(cPadFlag_Minus |
                                cPadFlag_Select, hold & KPAD_UC_BUTTON_MINUS);
                mPadHold.change(cPadFlag_Home,   hold & KPAD_UC_BUTTON_HOME);
                mPadHold.change(cPadFlag_Right,  hold & KPAD_UC_BUTTON_RIGHT);
                mPadHold.change(cPadFlag_Left,   hold & KPAD_UC_BUTTON_LEFT);
                mPadHold.change(cPadFlag_Up,     hold & KPAD_UC_BUTTON_UP);
                mPadHold.change(cPadFlag_Down,   hold & KPAD_UC_BUTTON_DOWN);
                mPadHold.change(cPadFlag_1,      hold & KPAD_UC_BUTTON_STICK_L);
                mPadHold.change(cPadFlag_2,      hold & KPAD_UC_BUTTON_STICK_R);
            ...
            else
            {
                ...
                mPadHold.change(cPadFlag_A,      hold & KPAD_BUTTON_A);
                mPadHold.change(cPadFlag_B,      hold & KPAD_BUTTON_B);
                mPadHold.change(cPadFlag_1,      hold & KPAD_BUTTON_1);
                mPadHold.change(cPadFlag_2,      hold & KPAD_BUTTON_2);
                mPadHold.change(cPadFlag_Z,      hold & KPAD_BUTTON_Z);
                mPadHold.change(cPadFlag_C,      hold & KPAD_BUTTON_C);
                mPadHold.change(cPadFlag_Plus |
                                cPadFlag_Start,  hold & KPAD_BUTTON_PLUS);
                mPadHold.change(cPadFlag_Minus |
                                cPadFlag_Select, hold & KPAD_BUTTON_MINUS);
                mPadHold.change(cPadFlag_Home,   hold & KPAD_BUTTON_HOME);
                mPadHold.change(cPadFlag_Right,  hold & KPAD_BUTTON_RIGHT);
                mPadHold.change(cPadFlag_Left,   hold & KPAD_BUTTON_LEFT);
                mPadHold.change(cPadFlag_Up,     hold & KPAD_BUTTON_UP);
                mPadHold.change(cPadFlag_Down,   hold & KPAD_BUTTON_DOWN);
                ...
                if (mIsEnableCL && (status.dev_type == WPAD_DEV_CLASSIC || status.dev_type == WPAD_DEV_MPLS_CLASSIC))
                {
                    ...
                    if (cl_hold & WPAD_CL_BUTTON_A)     mPadHold.set(cPadFlag_A);
                    if (cl_hold & WPAD_CL_BUTTON_B)     mPadHold.set(cPadFlag_B);
                    if (cl_hold & WPAD_CL_BUTTON_X)     mPadHold.set(cPadFlag_X);
                    if (cl_hold & WPAD_CL_BUTTON_Y)     mPadHold.set(cPadFlag_Y);
                    if (cl_hold & WPAD_CL_BUTTON_PLUS)  mPadHold.set(cPadFlag_Plus |
                                                                     cPadFlag_Start);
                    if (cl_hold & WPAD_CL_BUTTON_MINUS) mPadHold.set(cPadFlag_Minus |
                                                                     cPadFlag_Select);
                    ...
                    if (cl_hold & WPAD_CL_BUTTON_LEFT)  mPadHold.set(cPadFlag_Left);
                    if (cl_hold & WPAD_CL_BUTTON_RIGHT) mPadHold.set(cPadFlag_Right);
